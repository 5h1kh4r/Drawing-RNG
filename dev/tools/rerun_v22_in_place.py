...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Regenerate v2 enrollments and reverifiy raw redraws through Draw2Seed v2.2."
    )
    ...
    parser.add_argument("--algorithm-version", default="draw2seed-v2.2-structural-consensus")
    parser.add_argument("--config-version", default="v2.2-structural-2026-08-01")
    parser.add_argument("--out", default=str(ROOT / "results" / "v22_in_place"))
    ...
    for attempt_type in sorted({row["attempt_type"] for row in verification_rows}):
        ...
        summary["by_attempt_type"][attempt_type] = {
            ...
            "v22_accepts": sum(bool(row["v22_accepted"]) for row in rows),
