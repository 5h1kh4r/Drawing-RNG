...
class DrawChaoticRNG:
    ...
    def _compute_entropy(img: np.ndarray) -> float:
        unique_values = np.count_nonzero(np.bincount(img.ravel(), minlength=256))
        return float(np.log2(max(unique_values, 1)))
...
def sha256_image(image_path: Path, num_bytes: int) -> bytes:
    data = load_image_bytes(image_path)
    digest = hashlib.sha256(data).digest()

    # Expand if needed (simple chaining)
    output = bytearray()
    counter = 0
...
def v1_rng(image_path: Path, num_bytes: int, warmup: int) -> bytes:
    rng = DrawChaoticRNG(image_path, warmup_rounds=warmup)
    return rng.random_bytes(num_bytes)
...
def hybrid_v2(image_path: Path, num_bytes: int, warmup: int) -> bytes:
    base = v1_rng(image_path, num_bytes, warmup)
