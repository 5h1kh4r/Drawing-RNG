...
@app.get("/collect")
def collect_page():
    return jsonify({"error": "Public build only exposes the enrollment page."}), 404
...
def save_prompt_sample():
    return jsonify({"error": "Prompt collection is disabled in the public enrollment build."}), 404
...
def save_enrollment():
    if not PUBLIC_ENABLE_SERVER_LOGGING:
        return jsonify({"ok": True, "storage": "disabled", "message": "Public demo server-side logging is disabled."})
    payload = request.get_json(force=True, silent=False)
    if not isinstance(payload, dict):
        return jsonify({"error": "JSON body must be an object"}), 400
