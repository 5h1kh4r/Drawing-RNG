...
def derive_seed_material(serialized: str, salt: str, out_bytes: int = 32) -> str:
    ...
    if not salt:
        raise ValueError("salt is required; use seed_derivation.new_public_salt() for enrollments")
...
def encode_json_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    salt = payload.get("salt") or payload.get("public_salt")
    if salt:
        result["seed_material_hex"] = derive_seed_material(result["serialized"], salt=str(salt), out_bytes=32)
    else:
        result["seed_material_hex"] = None
        result["seed_material_warning"] = "seed_material_not_derived_without_explicit_salt"
    return result
